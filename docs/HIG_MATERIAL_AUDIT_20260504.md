...
> **Total effort**: 約 212 時間（≒ 5.3 人週、1名で約 1.3 ヶ月）
...
- [ ] **3rd Party SDK Privacy** — Sentry / Supabase / Stripe / Google Auth の SDK 申告
...
### Phase 0 — リリースブロッカー（Week 1 / 16h）

ストア審査を通過させるための必須修正のみ。
...
| 0-4 | iOS PrivacyInfo.xcprivacy 追加 (Required Reason API 4 種) | `ios/App/App/PrivacyInfo.xcprivacy` | 1h |
| 0-5 | Android Edge-to-Edge + Predictive Back 有効化 | `android/app/src/main/AndroidManifest.xml`, `MainActivity.java` | 2h |
| 0-6 | **Native IAP 化（iOS / Android）** Stripe Web 廃止 → @capacitor-community/in-app-purchases or RevenueCat | `src/billing/index.ts`, 新規 `src/billing/native.ts` | 8h |
| 0-7 | 5 プラグインインストール + sync | `package.json`, `npx cap sync` | 1h |
| 0-8 | `setHtmlPlatformAttr()` を `main.tsx` で呼び出し | `src/main.tsx` | 0.5h |
| 0-9 | input font-size を 14px → 16px (iOS auto-zoom 回避) | `src/PlacementTest.css`, `src/Roleplay.css`, `src/styles/primitives.css` | 1h |

**注**: 0-6 は最も大きい工数。Stripe Web 課金は **Apple App Store で確実にリジェクト** されるため、ネイティブ IAP 移行が事実上必須。
...
**合計工数**: 約 **212 時間** ≒ 5.3 人週（1 名 / 約 1.3 ヶ月）
