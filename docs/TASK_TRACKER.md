...
| DF-F13 | デイリーフェルミが残数表示のみで上級者の手応え薄い | P1 | IN_PROGRESS（2026-06-02 Keita 承認＝GO。論点全確定: ①フィルタは母集合絞り込みのみ・本数ルール不変 ②入力中はフィルタdisable ③5ドメイン集約 ④ja難易度一次案で進める ⑤在庫薄許容 ⑥日跨ぎ保持。実装着手: fermiData.ts タグ追加＋DailyFermiScreen フィルタUI（dev-logic委譲）） | dev-logic＋content-creator | 2026-06-02 |
