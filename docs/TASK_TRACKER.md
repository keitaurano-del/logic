...
| FB-14 | フラッシュカードの可読性改善（`<br/>`等markup除去・文字サイズ拡大・長文整形） | P2 | IN_PROGRESS（2026-06-02 Keita実機報告＝カードに`<br/>`が文字表示され読みにくい・解説等の文字が小さすぎる。根因特定済: ①過去の`<br/>`入りbody由来カードがlocalStorageに残存＋②generateFromLesson(flashcardData.ts:316)がstep.contentのmarkupをstripせず3行抜き出し＋③fc3フォント小・行間詰まり・長文がaspect-ratio固定カードに収まらない。3層修正: 表示sanitize+生成sanitize+CSS可読性。dev-logic実装） | dev-logic | UX・読みやすさ |
| FB-15 | フェルミ模範解答が途中で切れる（max_tokens不足） | P2 | IN_PROGRESS（2026-06-02 Keita実機報告＝解説が「出発点：上位8社を『大手」で途中ブツ切れ。根因特定済: server/routes/fermi.ts:277 の `/feedback` 模範解答生成が `max_tokens:1400`(haiku-4-5)で頭打ち＋手順の多い問題で超過＋プロンプトが「答えの数字は最後の## Answerでだけ言え」構成のため切れると答え自体が消失＋stop_reason未チェックで切れたまま素通し。修正: max_tokens 1400→2400・stop_reason=max_tokens検知でフォールバック。haiku＋ユーザ起動の低頻度なのでトークン節約方針と非衝突。dev-logic実装） | dev-logic | UX・回答消失バグ |
