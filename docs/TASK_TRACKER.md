...
| LR-25 | AI 体験の底上げ（応答の SSE ストリーミング化#46／採点・添削だけ上位モデル化#47）| P2 | IN_PROGRESS（2026-06-13 Son: #47採点/添削の上位モデル化を main マージ済[server/config.ts に AI_MODEL_LIGHT/GRADING 集約・env可変、採点=writing-score/添削=fermi feedback を sonnet-4-5に、軽タスクはhaiku据置]。残=#46 SSEストリーミング化(フロント消費含む)は別途。tsc/vitest788/eslint green／旧:  | dev-logic |
...
| DF-F22 | フェルミ問題プールの拡充（新規問題を追加・現50問→62問、手薄な cost/flow 分野と難易度バランスを補強） | P2 | REVIEW（2026-06-13 林: Keita 依頼「新しい問題増やして」。fermiData.ts の FERMI_POOL_JA/EN＋FERMI_STATS_JA/EN に index 並走で各 +12（50→62問）。ja=日本文脈・en=グローバル/US文脈[DF-F19準拠]、cost 1→5・flow 3→6 で薄いセルを補強、難易度 basic16/standard31/advanced15。作問アンカーは「（参考）」/レンジの概数で捏造精度なし[DF-F24/LR-24準拠]、既存50問と題材重複なし、新規分に全角／なし。tsc/build/eslint(.)0err/vitest 788 全 green。push/PR/merge は Keita 承認待ち） | （未push） | content-creator＋dev-logic |
