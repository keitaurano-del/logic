...
| AF-06 | アプリDLサイズ300MB削減：レッスンアセットをオンデマンド読込に | P1 | IN_PROGRESS（2026-06-01 Keita が設計案を承認＝GO。(a)計測＋(b)設計案 完了済→(c)実装フェーズ。**(c)第1スライス実装済（2026-06-01 自律ティック(林)）**＝resolveAssetUrl リゾルバ(src/lessonAssets.ts)+LazyAssetImage(プレースホルダ/lazy/onErrorで bundledフォールバック)を新規追加し HomeScreenV3/RoadmapScreenV3 のレッスン・コース画像参照を結線。フラグ VITE_REMOTE_LESSON_ASSETS が default OFF＝従来どおり bundled の /images/v3/*.png を使い**本番挙動は完全不変**。commit `f2d5a38`・green(tsc0/eslint0warn19/vitest33files530pass・新規lessonAssets14)・push/Render本番deploy済・Android main push自動配信。**第2スライス完了(2026-06-01 林)**＝結線漏れだった lesson-*.png ヒーロー(LessonStoriesScreen HeroImage)＋バッジ(TitleBadgeSheet/RankUpModal/ProfileScreenV3)＋ジャーナル推奨コース／保存サムネのバンドルPNG参照も resolveAssetUrl 結線（計8ファイル・ラップのみで挙動不変）・green(tsc0/eslint0/vitest34files554pass)・commit `846ceb3`・push/Render本番deploy。残=Storage公開バケットへPNGアップロード+フラグON+実AABサイズ計測（headless不可・要CI/実機検証環境）） | dev-logic（実装）+ designer（アセット最適化調査） | Keita 承認・着手 2026-06-01 |
