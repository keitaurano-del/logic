...
| FB-11 | 保存アイテムのフォルダ分け（FB-09 から分離） | P3 | IN_PROGRESS（2026-05-31 自律ティック(林)着手。ローカル(localStorage)完結のフォルダ整理機能を実装中＝端末間同期は DB schema migration=Keita承認要のため分離） | dev-logic | 低（DB schema migration 要） |
