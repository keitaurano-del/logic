...
| LR-4 | プライバシーポリシー全面改訂（健康データ／ジャーナルの Anthropic 送信／Sentry・Google TTS 委託先／属性PII／既存 LEGAL_REVIEW 差分を収集・委託先表に反映）#31,#33,#34,#35 | P0 | IN_PROGRESS（2026-06-10 Son: ブランチ feat/p0-legal-privacy-lr4。privacy(ja/en)に健康データ(Health Connect歩数/睡眠→Supabase米国)・ジャーナル→Anthropic・TTS・Sentry・属性PIIを委託先/所在国つきで開示(実コード裏取り)、account-deletion統一＋アプリ内削除導線追記、日付更新。残＝特商法の価格/事業者情報/トライアルは Keita 確定値待ち[HTMLコメントに確認メモ]。コード非変更・tsc green） | dev-logic＋Keita |
