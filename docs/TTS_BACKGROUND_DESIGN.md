...
## 8. 手動確認手順 — TTS 音声カスタマイズ / コース一覧プレビュー (2026-05-25 追加)

### 8.1 コース一覧の音声プレビュー (RoadmapScreenV3)

1. アプリ起動 → タブバー「コース」(Roadmap) を開く
2. グループ別コースカード（「論理的に考える」等）の右上のブックマークの下に▶ボタンが出ているはず
3. ▶ボタンタップ → そのコース紹介（タイトル + description + レッスン数 + レベル）が読み上げられる
4. 別カードの▶を押すと旧再生は止まり、新カードの紹介が始まる（排他制御）
5. 再生中のボタンは ■ (停止アイコン、brand 色) に変わる。再度タップで停止
6. レッスン詳細画面に進んで TTS モードを起動した場合も、こちらが優先されてコース紹介は止まる

### 8.2 ボイス表示の改善 (TtsControlPanel)

- レッスン詳細 → ヘッドホンアイコンで TTS モード ON → 下部パネルの「音声」プルダウンを開く
- 「自動選択」「女性の声」「男性の声」のショートカット直下に、全 voice が「Kyoko · 女性 · ja-JP」のような形式で並ぶ
- 旧仕様では「日本語-日本」のような lang 表示のみで複数 voice が区別できなかったが、今回 voice.name (端末固有) + 性別ラベル + lang の 3 要素表示に統一

### 8.3 ピッチ調整

- TtsControlPanel に「声の高さ: 低め / 普通 / 高め」のボタン群が追加されている
- 各ボタンタップで `SpeechSynthesisUtterance.pitch` が 0.75 / 1.0 / 1.25 に切り替わる
- 設定は localStorage `logic-tts-pitch` に保存され、次回起動・別レッスンでも維持される
- 再生中に変更した場合は現スライドを新しいピッチで即時再起動する

### 8.4 端末別の留意

- iOS: AVSpeechSynthesizer の pitch は連続的に効くが、Kyoko/Otoya 等の voice 自体が固定音域のため極端な変化は出にくい
- Android: BBCode TTS engine によっては pitch を無視する実装もあり、その場合は通常音で再生される (no-op)
- Web Speech API: voice によっては rate/pitch 共に無視するケースがある (Chrome の Google voice 等)。受け入れる仕様で fallback はしない
