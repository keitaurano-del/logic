/**
 * Generate all lesson thumbnails (v2 — hand-drawn notebook style)
 * via Gemini API.
 *
 * Usage:
 *   npx tsx scripts/generate-lesson-thumbnails-v2.ts                # all 49
 *   npx tsx scripts/generate-lesson-thumbnails-v2.ts --only=lesson-21,lesson-22
 *   npx tsx scripts/generate-lesson-thumbnails-v2.ts --skip-existing
 *   npx tsx scripts/generate-lesson-thumbnails-v2.ts --model=imagen-4.0-generate-001
 *   npx tsx scripts/generate-lesson-thumbnails-v2.ts --concurrency=3
 *   npx tsx scripts/generate-lesson-thumbnails-v2.ts --out=tmp/lesson-batch
 *
...
import { LESSON_PROMPTS, buildPrompt, type LessonPromptEntry } from './lessonPromptsV2.ts'
...
async function main() {
  ...
  const allJobs = LESSON_PROMPTS.filter((e: LessonPromptEntry) => !args.only || args.only.includes(e.slug))
  if (allJobs.length === 0) {
  ...
  if (failures.length) {
    ...
    console.log(`re-run failed: npx tsx scripts/generate-lesson-thumbnails-v2.ts --only=${failures.join(',')}`)
  }
}
