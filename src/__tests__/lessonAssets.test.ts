...
describe('lessonAssets / resolveAssetUrl', () => {
  const LOCAL = '/images/v3/lesson-20.webp'
  ...
  describe('remotable asset types (png/webp) vs non-remotable (svg)', () => {
    it('builds remote URL from webp local path when flag is on', () => {
      const webp = '/images/v3/lesson-20.webp'
      expect(resolveAssetUrl(webp, { remoteEnabled: 'on', baseUrl: BASE })).toBe(
        `${BASE}/lesson-20.webp`,
      )
    })

    it('builds remote URL from nested webp path', () => {
      const webp = '/images/v3/badges/badge-zenith.webp'
      expect(resolveAssetUrl(webp, { remoteEnabled: 'on', baseUrl: BASE })).toBe(
        `${BASE}/badge-zenith.webp`,
      )
    })

    it('returns svg localPath unchanged when flag is on (svg is not remotable)', () => {
