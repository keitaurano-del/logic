...
const numeracyLesson401: LessonData = {
  ...
  steps: [
    ...
    {
      type: 'quiz',
      explanation: 'Showing both growth rate and absolute amount, plus scale gap, is honest framing. Option 1 hides the tiny base, option 3 hides the relative momentum, option 4 vaguely averages everything and avoids specifics — a classic dodge.',
      question: 'Company A\'s profit grew $1M → $2M YoY. In the same period, Company B grew $100M → $105M. Which framing is most honest?',
      options: [
        { label: 'A grew 2× — leading the industry in pace', correct: false },
        { label: 'A grew 100% vs B\'s 5%, but B\'s profit is still ~50× larger in absolute terms', correct: true },
        { label: 'A is healthy with $1M added over the year', correct: false },
        { label: 'Both A and B are growing above the industry average', correct: false },
      ],
    },
    {
      type: 'quiz',
      explanation: 'When most of the population is vaccinated, absolute deaths among vaccinated naturally exceed unvaccinated. You must compare rates (per vaccinated vs. per unvaccinated) — without that, causation appears reversed. Options 2-4 are about chart styling, an entirely different axis from the "rate vs absolute count" problem here.',
      question: 'A claim is presented: "Deaths among vaccinated > deaths among unvaccinated." What is the most concerning issue?',
      options: [
        { label: 'It ignores vaccination-rate differences and compares absolute counts instead of rates', correct: true },
        { label: 'The Y-axis on the supporting chart is not at zero', correct: false },
        { label: 'The chart uses a logarithmic Y-axis', correct: false },
        { label: 'The comparison time window is too short to show signal', correct: false },
      ],
    },
  ],
}
...
const numeracyLesson405: LessonData = {
  ...
  steps: [
    ...
    {
      type: 'quiz',
      explanation: 'The mean of 20M is dragged by one person\'s 90M and misrepresents reality. The median 3M describes "the middle person\'s savings." For skewed data (income, housing, savings), median is the rule. Option 4 is a non-standard custom calculation with no statistical meaning — a typical "looks rigorous but isn\'t" trap.',
      question: '5 people\'s savings: 1M, 2M, 3M, 4M, 90M yen. Which measure most reflects reality?',
      options: [
        { label: 'Mean (20M) — reflects all data points', correct: false },
        { label: 'Median (3M) — middle value, robust to outliers', correct: true },
        { label: 'Mode — no value repeats, so undefined', correct: false },
        { label: 'The remainder after subtracting the max from the average', correct: false },
      ],
    },
    {
      type: 'quiz',
    ...
    {
      type: 'quiz',
      explanation: 'Mean > median signals a right-skewed distribution. A small number of high earners pulls the mean up. Option 2 describes left skew (opposite direction), option 3 is the "doubt the data" dodge, option 4 dismisses the median entirely — a common misunderstanding. Reality sits closer to 5M.',
      question: 'A dataset has mean income 8M and median income 5M. What is the most plausible interpretation?',
      options: [
        { label: 'Right-side (high-income) outliers inflate the mean', correct: true },
        { label: 'Left-side (low-income) outliers depress the mean', correct: false },
        { label: 'There is most likely a calculation error somewhere', correct: false },
        { label: 'The mean is always more trustworthy than the median', correct: false },
      ],
    },
  ],
}
...
const numeracyLesson406: LessonData = {
  ...
  steps: [
    ...
    {
      type: 'quiz',
      explanation: 'Failed entrepreneurs may also have been early risers. With only survivors in the sample, you can\'t tell "success cause" from "common trait." Option 1 questions correlation strength but the very comparison is impossible without failures, option 3 blames size (not the structural flaw), option 4 is off-axis. Failure-group comparison is essential.',
      ...
      options: [
        { label: 'The correlation between early rising and success is statistically weak', correct: false },
        { label: 'No data on failed founders is included (survivorship bias)', correct: true },
        { label: '100 founders is too small a sample to draw conclusions', correct: false },
        { label: 'There is no comparison data on non-founders (employees)', correct: false },
      ],
    },
    {
      type: 'quiz',
      explanation: 'Even if departments show no gender gap, men concentrating in "high-promotion-rate departments" makes the company-wide rate higher for men. Option 1 evades by blaming the data, option 3 introduces a movement story unrelated to the structure, option 4 dismisses a clearly significant pattern. Sample composition warps aggregation — that\'s Simpson\'s paradox.',
      question: 'No gender gap by department, but men have a higher promotion rate company-wide. The most plausible interpretation?',
      options: [
        { label: 'A data-aggregation error somewhere in the pipeline', correct: false },
        { label: 'Men cluster in departments with higher promotion rates (Simpson\'s paradox)', correct: true },
        { label: 'Men are moving between departments more frequently', correct: false },
        { label: 'The gap is statistically meaningless noise', correct: false },
      ],
    },
    {
      type: 'quiz',
